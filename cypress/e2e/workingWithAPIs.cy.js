...
it('modify api response', () => {
    cy.intercept('GET', '**/articles*', req => {
        req.continue( res => {
            res.body.articles[0].favoritesCount = 8888888
            res.send(res.body)
        })
    })
    cy.loginToApplication()
    cy.get('app-favorite-button').first().should('contain.text', '8888888')
})
...
it('delete article', () => {
    ...
    cy.request({
        ...
        cy.request({
            ...
            body: {
                "article": {
                    "title": "Test title Cypress delete test1",
            ...
            expect(response.body.article.title).to.equal('Test title Cypress delete test1')
        })
    })
    cy.loginToApplication()
    ...
    cy.get('app-article-list').should('not.contain.text', 'Test title Cypress delete test1')
